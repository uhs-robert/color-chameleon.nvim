...
### Auto-switch by directory (e.g., mounted/remote filesystems)

You can also automatically switch colorschemes based on your current working directory while preserving your previous theme:

```lua
-- State to track colorscheme switching
local prev_colorscheme = nil
local active_override = false
...
-- Auto-switch colorscheme based on directory
vim.api.nvim_create_autocmd("DirChanged", {
  callback = function()
    local cwd = realpath(vim.fn.getcwd())
    local mount_path = realpath("~/mnt/")

    -- Entering special directory
    if cwd:sub(1, #mount_path) == mount_path and not active_override then
      prev_colorscheme = vim.g.colors_name
      active_override = true
      vim.cmd.colorscheme("oasis-mirage")

    -- Leaving special directory
    elseif cwd:sub(1, #mount_path) ~= mount_path and active_override then
      active_override = false
      if prev_colorscheme then
        vim.cmd.colorscheme(prev_colorscheme)
      end
      prev_colorscheme = nil
    end
  end,
  desc = "Switch colorscheme for mounted/remote directories"
})
```

**Customize it:**
- Change `"~/mnt/"` to any path pattern you want to detect
- Change `"oasis-mirage"` to your preferred theme for that location
- Add multiple conditions for different directories or combine with `is_remote`/`is_root` checks
